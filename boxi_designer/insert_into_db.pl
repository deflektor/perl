...
sub identify_reports()
{
  my $a_rep_name= shift(@_);
  my $report_date,$report_days, $sth, $dbh;
  my @row;

  $report_days = 0;
  $dbh = DBI->connect($dsn, $username, $auth, \%attr);
  
  $sth = $dbh->prepare('select max(update_date) update_date, trunc(sysdate)-max(update_date) diff_days from ADMIN.BOXI_DATAPROVIDER where report_name=? group by report_name order by report_name asc');
  $sth->bind_param(1, $a_rep_name);
  $sth->execute();
#  $sth->execute($a_rep_name);
